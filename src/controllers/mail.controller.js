...
export const sendApplicationMail = asyncHandler(async (req, res) => {
    const userId = getAuthenticatedUserId(req);
    const mailBody = validateMailBody(req.body);
...
export const scheduleApplicationMail = asyncHandler(async (req, res) => {
    const userId = getAuthenticatedUserId(req);
    const mailBody = validateMailBody(req.body);
...
export const getScheduledMails = asyncHandler(async (req, res) => {
    const userId = getAuthenticatedUserId(req);
    const scheduledMails = await ScheduledMail.find({ user: userId })
        .populate({
        path: 'application',
        select: 'companyName companyWebsite recruiterName recruiterEmail jobTitle jobUrl location workMode source applicationType status priority subject emailBody appliedAt lastEmailSentAt followUpDate notes tags createdAt updatedAt',
    })
        .sort({ scheduledAt: 1 });
    res.status(200).json({
        success: true,
